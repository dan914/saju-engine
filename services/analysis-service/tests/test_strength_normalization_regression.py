...
class TestStrengthNormalizationRegression:
    ...
    def test_negative_strength_score_classification(self):
        ...
        birth_context = {"birth_dt": "2000-09-14T10:00:00", "gender": "M", "timezone": "Asia/Seoul"}
    ...
    def test_orchestrator_normalization_functions(self):
        ...
        assert orchestrator._normalize_strength_score(-100) == pytest.approx(
            0.0
        ), "극신약 (-100) should normalize to 0.0"
        assert orchestrator._normalize_strength_score(-22) == pytest.approx(
            0.39
        ), "신약 (-22) should normalize to 0.39"
        assert orchestrator._normalize_strength_score(0) == pytest.approx(
            0.5
        ), "중화 (0) should normalize to 0.5"
        assert orchestrator._normalize_strength_score(50) == pytest.approx(
            0.75
    ...
    def test_boundary_cases(self):
        ...
        # Test clamping
        assert orchestrator._normalize_strength_score(-200) == pytest.approx(
            0.0
        ), "Should clamp to 0.0"
        assert orchestrator._normalize_strength_score(200) == pytest.approx(
            1.0
    ...
    def test_yongshin_selector_multi_layer_defense(self):
        ...
        assert selector._get_strength_bin(strength_with_bin) == "weak", "bin should take priority"
        ...
        assert (
            selector._get_strength_bin(strength_normalized_only) == "weak"
        ...
        assert (
            selector._get_strength_bin(strength_raw_only) == "weak"
        ), "score_raw should be auto-normalized if score_normalized missing"

        # Test backward compatibility with deprecated "score" field
        strength_legacy = {"score": -22.0, "type": "신약"}  # Old format
        assert (
            selector._get_strength_bin(strength_legacy) == "weak"
    ...
    def test_grade_bin_mismatch_guard(self):
        ...
        assert bin_from_grade == "weak", "bin should come from grade_code (신약), not buggy score"
