...
# Utilities
SEASON_BY_BRANCH = {"寅":"봄","卯":"봄","辰":"장하","巳":"여름","午":"여름","未":"장하","申":"가을","酉":"가을","戌":"장하","亥":"겨울","子":"겨울","丑":"장하"}
...
class MasterOrchestrator:
    ...
    def _call_strength(self, pillars: Dict[str,str], season: str) -> Dict[str,Any]:
        ...
        for func in [getattr(self.strength, "evaluate", None),
                    getattr(self.strength, "run", None),
                    getattr(self.strength, "__call__", None)]:
            if callable(func):
                out = func(pillars=pillars, season=season) if func.__code__.co_argcount>=2 else func(pillars)
                return out if isinstance(out, dict) and "phase" in (out.get("strength", out)).get("phase","") or "phase" in out else out
    ...
    def _call_relations(self, pillars: Dict[str,str]) -> Dict[str,Any]:
        for func in [getattr(self.relations, "evaluate", None),
    ...
    def _call_climate(self, pillars: Dict[str,str], season: str, strength: Dict[str,Any]) -> Dict[str,Any]:
        # climate may need season & strength; pass both, fallback progressively
        for func in [getattr(self.climate, "evaluate", None),
    ...
    def _call_luck(self, pillars: Dict[str,str], birth_ctx: Dict[str,Any]) -> Dict[str,Any]:
        for name in ["compute", "compute_start_age", "run", "__call__"]:
            ...
            if callable(func):
                try:
                    return func(pillars=pillars, birth_dt=birth_ctx.get("birth_dt"), gender=birth_ctx.get("gender"))
                except TypeError:
                    return func(pillars, birth_ctx)
        raise RuntimeError("LuckCalculator has no callable entrypoint")

    def _call_yongshin(self, day_master: str, strength: Dict[str,Any], relations: Dict[str,Any], climate: Dict[str,Any]) -> Dict[str,Any]:
        for func in [getattr(self.yongshin, "select", None),
                     getattr(self.yongshin, "run", None),
                     getattr(self.yongshin, "__call__", None)]:
            if callable(func):
                try:
                    return func(day_master=day_master, strength=strength, relations=relations, climate=climate)
                except TypeError:
                    return func({"day_master":day_master,"strength":strength,"relations":relations,"climate":climate})
    ...
    def analyze(self, pillars: Dict[str,str], birth_context: Dict[str,Any]) -> Dict[str,Any]:
        ...
        elements = strength_ctx.get("elements") or strength.get("elements") or strength.get("strength_elements") or {}
        ...
        enriched = self.korean.enrich({
            "pillars": pillars, "season": season,
            "core": {"strength":strength, "relations":relations, "climate":climate, "yongshin":yongshin, "luck":luck},
            "stage3": s3
        })
        ...
        out = {
            "meta": {"orchestrator_version": self.version, "timestamp": datetime.utcnow().isoformat()+"Z"},
            "pillars": pillars, "birth_context": birth_context,
            "season": season, "day_master": day_stem,
            "core": {"strength":strength, "relations":relations, "climate":climate, "yongshin":yongshin, "luck":luck},
            "stage3": s3,
