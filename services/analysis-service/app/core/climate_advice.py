...
class ClimateAdvice:
    ...
    def _match(self, ctx: dict) -> Tuple[str, str]:
        ...
        for row in self.policy["advice_table"]:
            w = row["when"]
            if "season" in w and season not in w["season"]: continue
            if "strength_phase" in w and phase not in w["strength_phase"]: continue
            ok = True
            if "balance" in w:
                for k,v in w["balance"].items():
                    if bal.get(k) != v: ok=False; break
            if not ok: continue
            if "imbalance_flags" in w:
                if not all(f in (flags or []) for f in w["imbalance_flags"]): continue
