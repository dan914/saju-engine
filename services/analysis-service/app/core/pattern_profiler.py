...
class PatternProfiler:
    def __init__(self, policy_file: str = "pattern_profiler_policy_v1.json"):
        ...
        self.policy = p
        self.tags_catalog = set(p["tags_catalog"])

    def _match_when(self, when: dict, ctx: dict) -> bool:
        if "strength.phase_in" in when and _get(ctx,"strength.phase") not in set(when["strength.phase_in"]): return False
        ...
        if "relation.flags_any" in when:
            if not set(when["relation.flags_any"]).intersection(set(_get(ctx,"relation.flags",[]) or [])): return False
        if "yongshin.primary_in" in when and _get(ctx,"yongshin.primary") not in set(when["yongshin.primary_in"]): return False
        ...
        if "climate.flags_any" in when:
            if not set(when["climate.flags_any"]).intersection(set(_get(ctx,"climate.flags",[]) or [])): return False
        if "luck_flow.trend_in" in when and _get(ctx,"luck_flow.trend") not in set(when["luck_flow.trend_in"]): return False
        if "gyeokguk.type_in" in when and _get(ctx,"gyeokguk.type") not in set(when["gyeokguk.type_in"]): return False
        return True

    def run(self, ctx: dict) -> dict:
        tags, briefs = [], {"one_liner":"", "key_points":[]}
        for r in self.policy["rules"]:
            if self._match_when(r["when"], ctx):
                for t in r["emit"]["tags"]:
                    if t in self.tags_catalog and t not in tags: tags.append(t)
                bt = r["emit"].get("brief_templates")
                if bt:
                    if "one_liner" in bt and not briefs["one_liner"]: briefs["one_liner"] = bt["one_liner"]
                    if "key_points" in bt: briefs["key_points"].extend(bt["key_points"])
        return {
