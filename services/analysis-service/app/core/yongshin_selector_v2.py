...
class YongshinSelector:
    ...
    def _climate_yongshin(self, season: str) -> Dict[str, Any]:
        ...
        return {"primary": primary, "candidates": rule["candidates"], "weight": rule["primary_weight"], "rule_id": f"climate_{season}"}

    def _eokbu_yongshin(self, day_elem: str, strength_bin: str, elements_dist: Dict[str,float] = None) -> Dict[str, Any]:
        ...
        def deficit_gain(elem_en:str) -> float:
            if not elements_dist: return 0.0
    ...
    def _integrated_scores(self, day_elem: str, season: str, strength_bin: str, elements_dist: Dict[str,float], climate_elem: str) -> Dict[str,float]:
        # start at 0
        ...
        res = self._resource_elem(day_elem); comp = day_elem
        out = self._output_elem(day_elem); wea = self._wealth_elem(day_elem); off = self._official_elem(day_elem)
        if res: scores[res] += W["resource"]
        if comp: scores[comp] += W["companion"]
        if out: scores[out] += W["output"]
        if wea: scores[wea] += W["wealth"]
        if off: scores[off] += W["official"]
    ...
    # --- public ----------------------------------------------------------
    def select(self, day_master: str, strength: Dict[str,Any], relations: Dict[str,Any], climate: Dict[str,Any], elements_dist: Dict[str,float]=None) -> Dict[str,Any]:
        day_elem = STEM_TO_ELEM.get(day_master)
        season = climate.get("season")
        strength_bin = strength.get("bin") or ("weak" if (strength.get("score_normalized",0.5) < 0.4) else ("strong" if strength.get("score_normalized",0.5) >= 0.6 else "balanced"))
        ...
        # integrated
        climate_elem_en = {"목":"wood","화":"fire","토":"earth","금":"metal","수":"water"}.get(c_part["primary"]) if c_part["primary"] else None
        scores = self._integrated_scores(day_elem, season, strength_bin, elements_dist or {}, climate_elem_en)
        ...
        # package
        integrated = {
            "primary": {"elem_ko": ELEM_TO_KO[primary_en], "elem": primary_en, "score": round(primary_sc,3)},
            "secondary": {"elem_ko": ELEM_TO_KO[secondary_en], "elem": secondary_en, "score": round(secondary_sc,3)},
            "scores": {k: round(v,3) for k,v in scores.items()},
            "confidence": round(conf,3)
        }
        split = {
            "climate": {"primary": c_part["primary"], "candidates": c_part["candidates"], "rule_id": c_part["rule_id"]},
            "eokbu": { "primary": ELEM_TO_KO.get(e_part["primary"], e_part["primary"]),
