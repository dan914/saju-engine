...
def load_terms_for_year(year: int, use_sajulite: bool = False, use_refined: bool = False) -> list:
    ...
    if use_refined:
        data_dir = (
            Path(__file__).resolve().parents[1] / "data" / "canonical" / "terms_sajulite_refined"
        )
    elif use_sajulite:
...
def calculate_pillars(
    birth_dt: datetime, tz_str: str, use_sajulite: bool = False, use_refined: bool = False
) -> dict:
...
def run_tests(use_sajulite: bool = False, use_refined: bool = False):
    ...
    for tc in test_cases:
        ...
        try:
            ...
            results.append({"id": test_id, "label": label, "status": status, "pillars": output})
