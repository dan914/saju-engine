...
def analyze_range(start_year: int, end_year: int, source_name: str):
    ...
    if stats['total_comparisons'] > 0:
        ...
        print(f"\nDistribution:")
        print(f"  Identical (0 min):        {stats['identical']:4d} ({100*stats['identical']/stats['total_comparisons']:.1f}%)")
        print(f"  Within 1 minute:          {stats['within_1_min']:4d} ({100*stats['within_1_min']/stats['total_comparisons']:.1f}%)")
        print(f"  Within 15 minutes:        {stats['within_15_min']:4d} ({100*stats['within_15_min']/stats['total_comparisons']:.1f}%)")
        print(f"  Within 60 minutes:        {stats['within_60_min']:4d} ({100*stats['within_60_min']/stats['total_comparisons']:.1f}%)")
        print(f"  Over 60 minutes:          {stats['over_60_min']:4d} ({100*stats['over_60_min']/stats['total_comparisons']:.1f}%)")
        ...
        if stats['max_diff']:
            ...
            print(f"  {source_name}: {md['timestamps']['sl' if source_name == 'SKY_LIZARD' else 'kfa']}")
