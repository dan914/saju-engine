...
def test_midnight_transitions():
    ...
    for test_dt in test_cases:
        ...
        print(f"{status} | Input: {test_dt.strftime('%Y-%m-%d %H:%M')} | "
    ...
    print("=" * 130)
    print(f"{'Hour':<6} {'Total':<8} {'子時 Applied':<15} {'子時 Not Applied':<20} {'Expected Behavior':<30}")
    print("-" * 130)
    ...
    for hour in sorted(results_by_hour.keys()):
        ...
        print(f"{hour:02d}:xx  {stats['total']:<8} "
