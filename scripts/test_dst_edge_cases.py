...
def test_edge_cases():
        ...
        try:
            ...
            if 'expected_dst' in case:
                if result['dst_applied'] == case['expected_dst']:
                    ...
                    status = "❌ FAIL"
                    details.append(f"DST:❌ expected {case['expected_dst']}, got {result['dst_applied']}")
            ...
            if 'expected_offset' in case:
                ...
                if abs(actual_offset - case['expected_offset']) < 0.1:
                    ...
                    status = "❌ FAIL"
                    details.append(f"Offset:❌ expected {case['expected_offset']}, got {actual_offset}")
            ...
            if 'expected_lmt' in case:
                if result['lmt_offset'] == case['expected_lmt']:
                    ...
                    status = "❌ FAIL"
                    details.append(f"LMT:❌ expected {case['expected_lmt']}, got {result['lmt_offset']}")
    ...
    print(f"Passed:             {passed}/{passed + failed} ({100*passed/(passed+failed) if passed+failed > 0 else 0:.1f}%)")
