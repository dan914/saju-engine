...
def main():
    ...
    print(f"   ✓ Found data for {len(terms_by_year)} years ({min(terms_by_year.keys())}-{max(terms_by_year.keys())})")
    ...
    print(f"📊 Coverage: {min(terms_by_year.keys())}-{max(terms_by_year.keys())} ({len(terms_by_year)} years)")
